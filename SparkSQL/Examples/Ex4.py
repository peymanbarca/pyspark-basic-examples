...
if __name__ == "__main__":
    ...
    fetch_related_query = 'select id,city_id,region_id,neighbourhood_id,area_id,(EXTRACT(EPOCH FROM listing_date) * 1000)::bigint as listing_date,floor_area,listing_type_id,price,' \
                         ...
                         'from listing where expired is false and listing_type_id=1  order by listing_date desc limit {}'\
                            .format(sample_size)
    ...
    def contentBasedRecommenderScore(related_df_elem,related_data,cnt):
        ...
        related_data_V = related_data.value

        def compute_distance():

            distances = []
            related_data_sample = filter(lambda ROW: ROW['floor_area'] < 1.3*floor_area and ROW['floor_area'] > 0.7*floor_area, related_data_V)
        ...
        with open("OutputTotal.txt", "a") as text_file:
            text_file.write(json.dumps(result) + '  ---   ' + str(cnt)+ '\n')
    ...
    related_data = sc.broadcast(sample_listing_df.collect())

    # going from driver to worker nodes with loop of a dara frame and a shared broadcast variable and global variable from driver program
    # Remember that SparkContext can only be used on the driver, not in code that it run on workers
    sample_listing_df.foreach(lambda x: contentBasedRecommenderScore(x,related_data,accum))
